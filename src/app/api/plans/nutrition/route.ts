...
import {
  createNutritionPlan,
  getNutritionPlansForUser,
} from "@/lib/server/services/nutrition-plan.service";
...
export async function GET() {
  try {
    ...
    const plans = await getNutritionPlansForUser({ userId });
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error("Nutrition plan creation failed:", error);
    return NextResponse.json(
      { error: "Internal server error" },
      { status: 500 },
    );
